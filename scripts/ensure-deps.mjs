...
process.stderr.write('[youtube-viewer] Installing dependencies...\n');

if (!runNpm(['install', '--omit=dev', '--no-audit', '--no-fund'])) {
  process.stderr.write('[youtube-viewer] Dependency install failed. MCP tools may not work until resolved.\n');
  process.exit(0);
}

if (!binariesPresent()) {
  process.stderr.write('[youtube-viewer] Binaries missing — rebuilding ffmpeg-static and ytdlp-nodejs...\n');
  if (!runNpm(['rebuild', 'ffmpeg-static', 'ytdlp-nodejs']) || !binariesPresent()) {
    process.stderr.write('[youtube-viewer] Binary download failed. youtube_download, youtube_clip, and youtube_highlight_reel will not work.\n');
    process.exit(0);
  }
}
...
process.stderr.write('[youtube-viewer] Dependencies ready.\n');
