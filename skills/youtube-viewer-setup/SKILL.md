---
name: youtube-viewer:setup
description: Configure YouTube Viewer — search mode (anonymous/personalized) and plugin settings (defaults, language, locale)
---

# YouTube Viewer Setup

A guided walkthrough for configuring how the YouTube Viewer works. I present this as a friendly, conversational experience.
...
```
```
node "YOUR_PLUGIN_ROOT/scripts/config.mjs" --show
```
...
Present this to the user (use my own natural phrasing, but cover these points):

> **YouTube Viewer Setup**
>
> The YouTube Viewer has two modes:
>
...
1. If cookies exist (switching from personalized or broken state), run:
   ```
   node "YOUR_PLUGIN_ROOT/scripts/extract-cookies.mjs" --reset
   ```

2. Confirm to the user:
   > All set! Cookies have been removed. You'll need to **restart your Hermes Agent session** for the change to take effect (the MCP server caches cookies in memory, so a simple skill reload isn't enough).
   >
   > If you want personalized results later, just run `/youtube-viewer:setup` again.
...
>
> - **What I access:** YouTube authentication cookies only — used to identify your account
> - **How it works:** A separate Chrome window opens where you log into YouTube. I read cookies from that session — your main browser is never touched.
> - **Where they're stored:** Locally on your machine in the plugin data directory for cookies, plus a dedicated Chrome profile at `~/.youtube/chrome-profile/`. They never leave your device and are never sent to any third party.
> - **What they're used for:** Passed to YouTube's API so search results reflect your subscriptions, watch history, and preferences
> - **You're in control:** Run `/youtube-viewer:setup` anytime to switch back to anonymous mode or delete stored cookies
>
...
```
node "YOUR_PLUGIN_ROOT/scripts/extract-cookies.mjs"
```
...
> 2. **Check your email** — Google sends an authentication verification message. Confirm the login by clicking "Yes, it was me" in that email.
> 3. Let me know when you're signed in and have confirmed the email
>
> This is a one-time step — your login will persist for future sessions. Your main Chrome browser is not affected.

Wait for the user to confirm they've logged in and verified the email, then re-run:
```
node "YOUR_PLUGIN_ROOT/scripts/extract-cookies.mjs"
```

If it returns LOGIN_REQUIRED again, ask the user to double-check they're logged in on youtube.com in the Chrome window that opened (not their regular browser) and that they confirmed the email verification.
...
If I haven't already, run silently:
```
node "YOUR_PLUGIN_ROOT/scripts/config.mjs" --show
```
...
```
node "YOUR_PLUGIN_ROOT/scripts/config.mjs" --set <section.key> <value>
```

For example:
- "I want 30 search results by default" → `node "YOUR_PLUGIN_ROOT/scripts/config.mjs" --set search.defaultLimit 30`
- "Change language to Spanish" → `node "YOUR_PLUGIN_ROOT/scripts/config.mjs" --set innertube.language es` and `node "YOUR_PLUGIN_ROOT/scripts/config.mjs" --set transcript.defaultLanguage es`
...
>
> The MCP server reads settings at startup, so you'll need to **restart your Hermes Agent session** for changes to take effect.
>
...
```
node "YOUR_PLUGIN_ROOT/scripts/config.mjs" --reset
```

Confirm:
> All settings have been reset to defaults. Restart your Hermes Agent session for changes to take effect.
...
>
> If you ever want to switch back to anonymous or refresh your cookies, just run `/youtube-viewer:setup` again.

If still anonymous despite cookie extraction succeeding:
> The cookies were saved but the MCP server needs to restart to pick them up. Please **restart your Hermes Agent session**, then try a search — it should show personalized mode after that.
>
> If you ever want to switch back to anonymous or refresh your cookies, just run `/youtube-viewer:setup` again.
...
- Settings are stored in the plugin's persistent data directory alongside cookies.
- When changing settings, always remind the user to restart their Hermes Agent session for changes to take effect.
- **YouTube sends an authentication email** when you sign in from a new device. The user must check their inbox and confirm the login before cookie extraction succeeds.
